...
def retry_validation_failures(run_dir: Path, manifest: dict, log_file: Path, max_retries: int | None = None) -> int:
    ...
    - Creates a new retry chunk containing only the failed units
    - Rotates {step}_failures.jsonl to {step}_failures.jsonl.bak
    - Preserves hard failures (pipeline_internal) in the original failures file
    - Does NOT reset the original chunk — its validated output is preserved

    This avoids the chunk-level reset bug where all units (including those that
    already passed) would be reprocessed when only a few failed.
    ...
    for chunk_name in list(chunks.keys()):
        ...
        for step in pipeline:
            ...
            # Categorize failures
            validation_failures: list[tuple[dict, str]] = []
            hard_failures = []
            try:
                with open(failures_file) as f:
                    for line in f:
                        ...
                        try:
                            ...
                            if stage in validation_stages:
                                validation_failures.append((failure, line_s))
                            else:
            ...
            # Look up the correct input source for failed units
            ...
            # Build unit list for retry chunk (deduplicate by unit_id)
            units_for_retry = {}
            for failure, _line_s in validation_failures:
                unit_id = failure.get("unit_id")
                if not unit_id:
                    continue
                ...
                if not unit_data:
                    log_message(log_file, "WARN",
                        f"Could not find input for {unit_id} in {input_source}")
                    continue
                ...
                if unit_id not in units_for_retry:
                    ...
                    existing = units_for_retry[unit_id]["retry_count"]
                    if item_retry_count > existing:
                        units_for_retry[unit_id]["retry_count"] = item_retry_count

            retriable_unit_ids = set(units_for_retry.keys())
            unretryable_validation_lines = [
                line_s for failure, line_s in validation_failures
                if failure.get("unit_id") not in retriable_unit_ids
            ]
            archived_for_step = len(validation_failures) - len(unretryable_validation_lines)

            # Create retry chunk with only the failed units
            if units_for_retry:
                ...
                with open(units_file, "w") as f:
                    for unit_id, info in units_for_retry.items():
                        unit = info["unit"].copy()
                        unit["unit_id"] = unit_id
                        unit["retry_count"] = info["retry_count"] + 1
                        f.write(json.dumps(unit) + "\n")

                max_retry_count = max(u["retry_count"] for u in units_for_retry.values())
                chunks[retry_name] = {
                    "state": f"{step}_PENDING",
                    "batch_id": None,
                    "items": len(units_for_retry),
                    "valid": 0,
                    "failed": 0,
                    "retry_count": max_retry_count + 1,
                    "source_chunks": [chunk_name],
                    "retry_step": step,
                }

                log_message(log_file, "RETRY",
                    f"Created {retry_name} with {len(units_for_retry)} unit(s) "
                    f"from {chunk_name}/{step} for retry")

                next_retry_num += 1

            # If no validation failures were retriable, leave failures file intact.
            if archived_for_step <= 0:
                continue
            ...
            # Rewrite failures file with hard failures and unretryable validation failures.
            remaining_lines = hard_failures + unretryable_validation_lines
            if remaining_lines:
                with open(failures_file, 'w') as f:
                    for line_s in remaining_lines:
                        f.write(line_s + '\n')
            else:
                try:
            ...
            archived_total += archived_for_step

        # Recalculate failed count from remaining hard failures
        # (original chunk state is NOT reset — only hard failures remain)
        remaining = 0
        for step in pipeline:
    ...
    for chunk_name, chunk_data in list(chunks.items()):
        ...
        if status == "PENDING":
            ...
            if is_expression_step(config, step):
                ...
                expr_chunk_dir = run_dir / "chunks" / chunk_name
                needs_validation = step_has_validation(config, step)

                try:
                    # If validation rules exist, write to _results.jsonl first
                    expr_output_file = None
                    if needs_validation:
                        expr_output_file = expr_chunk_dir / f"{step}_results.jsonl"

                    valid, failed_count, _, _ = run_expression_step(
                        run_dir, chunk_name, step, step_config, config, manifest, log_file,
                        output_file=expr_output_file
                    )

                    # Run validation pipeline if step has validation rules
                    if needs_validation:
                        results_file = expr_chunk_dir / f"{step}_results.jsonl"
                        validated_file = expr_chunk_dir / f"{step}_validated.jsonl"
                        failures_file = expr_chunk_dir / f"{step}_failures.jsonl"
                        schema_path = get_schema_path(config, step, run_dir)

                        step_idx = pipeline.index(step) if step in pipeline else 0
                        if step_idx == 0:
                            step_input_file = expr_chunk_dir / "units.jsonl"
                        else:
                            prev_step = pipeline[step_idx - 1]
                            step_input_file = expr_chunk_dir / f"{prev_step}_validated.jsonl"

                        log_message(log_file, "EXPRESSION", f"{chunk_name}: Running validation for expression step '{step}'")
                        valid, failed_count = run_validation_pipeline(
                            results_file, validated_file, failures_file,
                            schema_path, config_path, step, log_file, chunk_name,
                            input_file=step_input_file,
                            timeout=get_subprocess_timeout(config)
...
def run_expression_step(
    ...
    Evaluates expressions from step config for each unit and writes to
    output file. By default writes to {step}_validated.jsonl; when the step
    has validation rules, the caller should pass output_file={step}_results.jsonl
    and run the validation pipeline separately.
    ...
    Args:
        ...
        output_file: Optional explicit output path; defaults to {step}_validated.jsonl
    ...
    validated_file = output_file if output_file else chunk_dir / f"{step}_validated.jsonl"
...
def step_has_validation(config: dict, step_name: str) -> bool:
    """Check if a step has validation rules defined in config.yaml."""
    return step_name in config.get("validation", {})
                ...
                for chunk_name in chunks_for_step:
                    ...
                    if is_expression_step(config, step):
                        ...
                        if needs_validation:
                            ...
                            log_message(log_file, "EXPRESSION", f"{chunk_name}: Running validation for expression step '{step}'")
                            valid, failed = run_validation_pipeline(
                                results_file, validated_file, failures_file,
                                schema_path, config_path, step, log_file, chunk_name,
                                input_file=step_input_file,
                                timeout=get_subprocess_timeout(config)
